function [PLV dRPvar MsKappa LHat LVar RPvar rlxtime SRPeps] = coupledNMM_wrapper_gen(R,Klist,dvar,omvar,sigvar,dflag,surflag)
if nargin<7
    surflag=0;
end
dt = R.dt;
...
A =[
    ...
    0     0     0.3   0];
% A = rand(4).*A;
sigA = [1 0.2 1 0.2]; % Noise input
Nr = size(A,2);
EImap = [1 -1 1 -1]; intmod = ones(Nr)+(2.*-eye(Nr));
A = (A.*(EImap)).*intmod;
if dflag == 1
...
% %         0     0     3     0];
    ...
    Da = D(:);
    yr = [120;173;111;73];
    y(:,1,:) = repmat(yr,1,1,max(Da)+1);
    yr = [2;10;20;1];
    y(:,2,:) = repmat(yr,1,1,max(Da)+1);
else
    D = zeros(Nr);
    y(:,1,1) = [120;173;111;73];
    y(:,2,1) = [2;10;20;1];
end
[par,H,T] = NMM_parameters(Nr,EImap);

sppart = 5; L = 0;
for i=5:numel(Klist)
    [ystore{1} tvec{1} rlxtime(:,i)] = fx_Nnode_NMM(dt,tt,Nr,Klist(i),A,par,H,T,sigvar,sigA,y,D,burn);
    if surflag ==1
